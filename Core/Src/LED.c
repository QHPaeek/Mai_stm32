...
#define NUM_LED 16
#define WS2812_HIGH 67
#define WS2812_LOW 23
uint8_t RGB_data_raw[48] = {0xff}; //16LED
uint32_t RGB_data_DMA_buffer[609] = {90};
...
void LED_refresh()
{
	//pwm计数90重载，设置为90即一直低电平，设置为67表示ws2812的高，设置为23表示ws2812的低
	//ws2812传输数据顺序要求是GRB,游戏下发数据为RGB,�??????要对调顺�??????
	for(uint8_t i = 0 ;i<16;i++)
	{
		for(uint8_t j = 0 ;j <8;j++)
		{
			RGB_data_DMA_buffer[(i*3)*8+j+224] = (gamma8[RGB_data_raw[i*3+1]] & (1<<j)) ? WS2812_HIGH:WS2812_LOW;
		}
		for(uint8_t j = 0 ;j <8;j++)
		{
			RGB_data_DMA_buffer[(i*3+1)*8+j+224] = (gamma8[RGB_data_raw[i*3]] & (1<<j)) ? WS2812_HIGH:WS2812_LOW;
		}
		for(uint8_t j = 0 ;j <8;j++)
		{
			RGB_data_DMA_buffer[(i*3+2)*8+j+224] = (gamma8[RGB_data_raw[i*3+2]] & (1<<j)) ? WS2812_HIGH:WS2812_LOW;
		}
	}
